...
class FHE:
    ...
    def decrypt(self, c):
        ...
        x = self.bfv.decode_int(x_poly, unit=self.unit)
        assert isinstance(x, int), "Decrypted value must be an integer"
        return x
    ...
    def encrypt_vec(self, x):
        ...
        c_x = np.empty_like(x, dtype=object)
    ...
    def decrypt_vec(self, c_x):
        ...
        x = np.empty_like(c_x, dtype=object)
        for i in range(c_x.shape[0]):
            print(type(c_x[i]))
            print(type(self.decrypt(c_x[i])))
            x[i] = self.decrypt(c_x[i])
        return c_x

    def add_vec(self, c_x, c_y):
        '''homomorphically adds two encrypted vectors element-wise'''
        c_z = np.empty_like(c_x, dtype=object)
        for i in range(c_x.shape[0]):
            c_z[i] = self.add(c_x[i], c_y[i])
        return c_z

    def add_vec_const(self, c, p):
        '''homomorphically adds two encrypted vectors element-wise'''
        c_z = np.empty_like(p, dtype=object)
        for i in range(p.shape[0]):
            c_z[i] = self.add_const(c[i], p[i])
        return c_z
    ...
    def dot_const(self, p, c):
        ...
        assert len(p) == len(c)
        mults = np.empty_like(p, dtype=object)
        for i in range(p.shape[0]):
    ...
    def dot(self, c_x, c_y):
        ...
        mults = np.empty_like(c_x, dtype=object)
        for i in range(c_x.shape[0]):
    ...
    def add_matrix_vector_const(self, p_vec, c_mat):  # add vector to rows
        '''homomorphically add a vector of integers to a 2d matrix of ciphertexts with vector '''
        c_z = np.empty_like(c_mat, dtype=object)
        for i in range(c_mat.shape[0]):
            c_z[i] = self.add_vec_const(c_mat[i], p_vec)
        return c_z
